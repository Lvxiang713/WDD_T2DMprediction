...
if __name__ == '__main__':    
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        op_list,w_list = fit(X_train,y_train,gamma,delta)
        op_list1.append(op_list)
        w_list1.append(w_list)
        if len(op_list)!=0:
            cv_final = CV_score(X_train,y_train,X_test,y_test,gamma,delta,op_list,w_list)
    ...
    with open(os.path.join(out_path,'DIpara.pk'), 'wb') as f:
