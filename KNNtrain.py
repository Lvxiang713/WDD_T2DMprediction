...
if __name__ == '__main__':    
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        X_train, X_test = normali(X_train, X_test,dealoutlier = False)
        X_train = np.expand_dims(X_train,axis=1)
        X_test = np.expand_dims(X_test,axis=1)
        y_train, y_test = y[train_index], y[test_index]
        op_list,w_list = fit(X_train,y_train,gamma)
        op_list1.append(op_list)
        w_list1.append(w_list)
        if len(op_list)!=0:
            cv_final = CV_score(X_train,y_train,X_test,y_test,gamma,op_list,w_list)         
    ...
    with open(os.path.join(out_path,'KNNpara.pk'), 'wb') as f:
